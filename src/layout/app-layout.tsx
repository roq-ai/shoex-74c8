...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'quality-control-specialist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ManufacturerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/manufacturers` : null),
    () => getManufacturers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Manufacturer',
      path: isTenantUser ? `/manufacturers/view/${data?.[0]?.id}` : '/manufacturers',
      entity: 'manufacturer',
