...
export const getManufacturers = async (query?: ManufacturerGetQueryInterface) => {
  const response = await axios.get(`/api/manufacturers${query ? `?${queryString.stringify(query)}` : ''}`);
